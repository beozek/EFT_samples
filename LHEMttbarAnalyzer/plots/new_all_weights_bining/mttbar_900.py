...
def overlay_histograms_normalized():
    ...
    # file1 = ROOT.TFile.Open("../700_900_root/lhe_700_900.root")
    file1 = ROOT.TFile.Open("../900_Inf_root/lhe_900Inf.root")

    # file2 = ROOT.TFile.Open("0_700.root")
    # file2 = ROOT.TFile.Open("700_900.root")
    file2 = ROOT.TFile.Open("900_Inf.root")
    ...
    hist2 = file2.Get("LHEMttbarAnalyzer/mttbar_eft").Clone("hist2")
    # hist3 = file3.Get("LHEMttbarAnalyzer/mttbar_eft").Clone("hist3")
    ...
    if N_gen1 == 0 or N_gen2 == 0:
        print("Error: One of the histograms has zero generated events. Cannot scale.")
        return

    scaling_factor1 = sigma3 / N_gen1
    scaling_factor2 = sigma3 / N_gen2
    ...
    hist2_rebinned.Draw("HIST")

    hist1_rebinned.Draw("HIST SAME")
    # hist3_rebinned.Draw("HIST SAME")

    legend = ROOT.TLegend(0.65, 0.75, 0.88, 0.88)
    legend.SetBorderSize(0)
    legend.SetFillStyle(0)
    legend.AddEntry(hist1_rebinned, "Mttbar (old)", "l")
    legend.AddEntry(hist2_rebinned, "Mttbar (new)", "l")
    # legend.AddEntry(hist3_rebinned, "Mttbar (900-Inf GeV)", "l")
    # legend.AddEntry(hist_combined, "Total Combined", "l")
    legend.Draw()

    max_y = max(hist1_rebinned.GetMaximum(), hist2_rebinned.GetMaximum())
    # hist1_rebinned.SetMaximum(max_y * 1.2)
    c1.SaveAs("mttbar_indv900_norm_log.png")
    c1.Clear()

    # hist_combined.Draw("HIST")
    # c1.SaveAs("mttbar_comb700.png")
